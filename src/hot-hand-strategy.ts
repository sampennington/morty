...
// const PURGE_WINDOW = 4;    // +/- steps around predicted crest
// const PURGE_PROBE_INTERVAL = 25; // deterministic probing cadence until phase is learned
// const CONFIDENCE_STREAK_FOR_X3 = 2; // how many correct crest hits before x3

// function mod(a: number, n: number) { return ((a % n) + n) % n; }
// function distMod(a: number, b: number, n: number) {
//   const d = Math.abs(mod(a - b, n));
//   return Math.min(d, n - d);
// }

// async function hotHandStrategy() {
...
//   console.log('🎯 HOT HAND + PURGE PHASE STRATEGY');
//   console.log('Rules: Probe x1 normally. After a WIN → stay and send x3.');
//   console.log('Purge has period=200; learn phase offset and only hit within a tight window.\n');
//   console.log('═'.repeat(60));

//   const status = await api.startEpisode();
//   console.log('\n✅ Episode started!\n');

//   let mortiesRemaining = status.morties_in_citadel;
//   let currentStep = status.steps_taken;

//   // Track last result per planet
//   const planetResults = new Map<Planet, boolean | null>([
//     [Planet.ON_A_COB, null],
//     [Planet.CRONENBERG, null],
//     [Planet.PURGE, null],
//   ]);

//   let currentPlanet: Planet | null = null;

//   // --- Purge phase model state ---
//   let purgeOffset: number | null = null;      // learned as (step % 200) when we first win on Purge
//   let purgeConfidence = 0;                    // # of confirmed crest hits
//   let lastPurgeProbeStep = -Infinity;         // for deterministic probing before we know offset

//   function isPurgeWindow(step: number) {
//     if (purgeOffset === null) return false;
//     const phase = mod(step, PURGE_PERIOD);
//     return distMod(phase, purgeOffset, PURGE_PERIOD) <= PURGE_WINDOW;
//   }

//   function shouldProbePurge(step: number) {
//     // Before phase learned: probe deterministically every N steps (not spamming)
//     if (purgeOffset !== null) return false;
//     return step - lastPurgeProbeStep >= PURGE_PROBE_INTERVAL;
//   }
...
//     // --- Decide planet & count ---
//     let chosenPlanet: Planet = Planet.ON_A_COB;
//     let desiredCount: 1 | 2 | 3 = 1; // default probe

//     const nextStep = currentStep; // decision is for this trip index

//     // 1) PURGE OVERRIDE: if we are in the predicted high window, take Purge
//     if (isPurgeWindow(nextStep)) {
//       chosenPlanet = Planet.PURGE;
//       desiredCount = (purgeConfidence >= CONFIDENCE_STREAK_FOR_X3) ? 3 : 1;
//       console.log(`\n🧠 Trip ${nextStep}: PURGE WINDOW → ${PLANET_NAMES[chosenPlanet]} (x${desiredCount}) [offset=${purgeOffset}, conf=${purgeConfidence}]`);
//     }
//     // 2) If phase unknown, probe Purge on a deterministic cadence
//     else if (shouldProbePurge(nextStep)) {
//       chosenPlanet = Planet.PURGE;
//       desiredCount = 1;
//       console.log(`\n🔎 Trip ${nextStep}: PURGE PHASE DISCOVERY PROBE → ${PLANET_NAMES[chosenPlanet]} (x1)`);
//       lastPurgeProbeStep = nextStep;
//     }
//     // 3) Otherwise use hot-hand on the current streak
//     else if (currentPlanet === null) {
//       chosenPlanet = Planet.ON_A_COB;
//       desiredCount = 1;
//       console.log(`\n🚀 Trip ${nextStep}: Initial PROBE -> ${PLANET_NAMES[chosenPlanet]} (x${desiredCount})`);
//     } else {
//       const lastResult = planetResults.get(currentPlanet);
//       if (lastResult === true) {
//         chosenPlanet = currentPlanet;
//         desiredCount = 3; // ride hot hand
//         console.log(`\n🔥 Trip ${nextStep}: HOT STREAK -> stay on ${PLANET_NAMES[chosenPlanet]} (x${desiredCount})`);
//       } else {
//         // switch logic: prefer a known recent winner; else untested; else random (not current)
//         let found = false;
//         for (const [planet, result] of planetResults.entries()) {
//           if (result === true && planet !== currentPlanet) {
//             chosenPlanet = planet; found = true;
//             console.log(`\n🔄 Trip ${nextStep}: LOSS → switch to previous winner ${PLANET_NAMES[chosenPlanet]} (x1 PROBE)`);
//             break;
//           }
//         }
//         if (!found) {
//           for (const [planet, result] of planetResults.entries()) {
//             if (result === null && planet !== currentPlanet) {
//               chosenPlanet = planet; found = true;
//               console.log(`\n🆕 Trip ${nextStep}: LOSS → explore untested ${PLANET_NAMES[chosenPlanet]} (x1 PROBE)`);
//               break;
//             }
//           }
//         }
//         if (!found) {
//           const otherPlanets = [Planet.ON_A_COB, Planet.CRONENBERG, Planet.PURGE].filter(p => p !== currentPlanet);
//           chosenPlanet = otherPlanets[Math.floor(Math.random() * otherPlanets.length)];
//           console.log(`\n🎲 Trip ${nextStep}: LOSS → try ${PLANET_NAMES[chosenPlanet]} (x1 PROBE)`);
//         }
//         desiredCount = 1;
//       }
//     }

//     // Enforce remaining morties
//     const mortyCount = Math.min(desiredCount, mortiesRemaining) as 1 | 2 | 3;

//     // --- Execute ---
//     const result = await api.sendThroughPortal(chosenPlanet, mortyCount);

//     // --- Update tracking ---
//     planetResults.set(chosenPlanet, result.survived);

//     // PURGE phase learning & confidence updates
//     if (chosenPlanet === Planet.PURGE) {
//       const phaseNow = mod(nextStep, PURGE_PERIOD);

//       if (result.survived) {
//         if (purgeOffset === null) {
//           // First confirmed crest — lock phase
//           purgeOffset = phaseNow;
//           purgeConfidence = 1;
//           console.log(`   🧩 Learned PURGE phase: offset=${purgeOffset} (step%${PURGE_PERIOD})`);
//         } else if (isPurgeWindow(nextStep)) {
//           purgeConfidence = Math.min(3, purgeConfidence + 1);
//         }
//       } else {
//         // Miss reduces confidence slightly; if repeatedly missing near window, widen or re-learn
//         if (purgeOffset !== null && distMod(phaseNow, purgeOffset, PURGE_PERIOD) <= PURGE_WINDOW + 1) {
//           purgeConfidence = Math.max(0, purgeConfidence - 1);
//         }
//         // Optional: if confidence collapses, forget and re-learn
//         // if (purgeConfidence === 0) { purgeOffset = null; lastPurgeProbeStep = nextStep; }
//       }
//     }
...
//     const successRate = (result.morties_on_planet_jessica / nextStep * 100).toFixed(1);
//     console.log(`   ${emoji} Result: ${result.survived ? 'SURVIVED' : 'DIED'} on ${PLANET_NAMES[chosenPlanet]} (x${mortyCount})`);
//     console.log(`   📊 Saved: ${result.morties_on_planet_jessica} Lost: ${result.morties_lost} | Citadel: ${result.morties_in_citadel} | Success: ${successRate}%`);
...
//   console.log('═'.repeat(60));
//   console.log(`✅ Morties that survived: ${finalStatus.morties_on_planet_jessica}`);
...
// hotHandStrategy()
//   .then(result => {
//     const rate = (result.morties_on_planet_jessica / 1000 * 100).toFixed(2);
//     console.log(`\n🎉 Final success rate: ${rate}%`);
//     if (parseFloat(rate) >= 90) {
//       console.log('🏆 ACHIEVEMENT UNLOCKED: 90%+ SUCCESS RATE!');
//     } else if (parseFloat(rate) >= 80) {
//       console.log('🥈 Great job! Getting close to 90%!');
//     } else if (parseFloat(rate) >= 70) {
//       console.log('🥉 Good progress! Need to refine the strategy.');
//     }
//   })
//   .catch(err => {
